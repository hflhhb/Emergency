...
namespace LeaRun.CustomerManage.Service
{
    ...
    /// 描 述：订单管理
    /// </summary>
    public class OrderService : Dao<OrderEntity>, IOrderService
    {
        ...
        public OrderService(DbContextBase dbcontext) : base(dbcontext)
        {
        ...
        public OrderEntity GetPrevOrNextEntity(string keyValue, int type)
        {
            ...
            if (type == 1)
            {
                entity = base.Queryable().Where(t => t.CreateDate >entity.CreateDate).OrderBy(t => t.CreateDate).FirstOrDefault();
            }
            else if (type == 2)
            {
                entity = base.Queryable().Where(t => t.CreateDate < entity.CreateDate).OrderByDescending(t => t.CreateDate).FirstOrDefault();
            }
            return entity;
        }
        #endregion
        ...
        public void SaveForm(string keyValue, OrderEntity orderEntity, List<OrderEntryEntity> orderEntryList)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    coderuleService.UseRuleSeed(orderEntity.CreateUserId, "", ((int)CodeRuleEnum.Customer_OrderCode).ToString());//占用单据号
                    //明细
                    foreach (OrderEntryEntity orderEntryEntity in orderEntryList)
                    {

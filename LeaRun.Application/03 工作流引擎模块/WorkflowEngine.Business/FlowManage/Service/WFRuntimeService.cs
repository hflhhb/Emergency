...
namespace LeaRun.WorkflowEngine.Service
{
    ...
    /// 描 述：工作流运行实例处理方法类
    /// </summary>
    public class WFRuntimeService :DaoFactory, WFRuntimeIService
    {
        private WFSchemeInfoIService wfSchemeInfoService = null;
        private WFSchemeContentIService wfSchemeContentService = null;

        private WFProcessInstanceIService wfProcessInstanceService = null;
        private WFProcessSchemeIService wfProcessSchemeService = null;
        private WFProcessOperationHistoryIService wfProcessOperationHistoryService = null;
        private WFProcessTransitionHistoryIService wfProcessTransitionHistoryService = null;
        private WFDelegateRuleIService wfDelegateRuleService = null;

        private IUserService userService = new UserService(LeaRun.Data.DbFactory.Base());//用户数据库操作类
        private IDepartmentService departmentService = new DepartmentService(LeaRun.Data.DbFactory.Base());
        private IOrganizeService organizeService = new OrganizeService(LeaRun.Data.DbFactory.Base());

        private IDataBaseLinkService dataBaseLinkService = new DataBaseLinkService(LeaRun.Data.DbFactory.Platform());
        ...
        public WFRuntimeService(DbContextBase dbcontext) : base(dbcontext)
        {
            wfSchemeInfoService = new WFSchemeInfoService(dbcontext);
            wfSchemeContentService = new WFSchemeContentService(dbcontext);

            wfProcessInstanceService = new WFProcessInstanceService(dbcontext);
            wfProcessSchemeService = new WFProcessSchemeService(dbcontext);
            wfProcessOperationHistoryService = new WFProcessOperationHistoryService(dbcontext);
            wfProcessTransitionHistoryService = new WFProcessTransitionHistoryService(dbcontext);
            wfDelegateRuleService = new WFDelegateRuleService(dbcontext);
        }
        ...
        private string GetFrmData(string DataBaseId, string tableName, string tableFiled, string processId)
        {
            ...
            try
            {
                ...
                DataTable dt = new DbContextBase(dataBaseLinkEntity.DbConnection).FindTable(sqlstr);

using LeaRun.PublicInfoManage.Business;
...
namespace LeaRun.PublicInfoManage.Controllers
{
    ...
    public class EmailController : MvcControllerBase
    {
        ...
        [HttpPost]
        [AjaxOnly]
        public ActionResult SteriskEmail(string senderId, string keyValue, int asterisk = 1)
        {
            bool isSenderSelf = OperatorProvider.Provider.Current().UserId == senderId;
            emailContentBLL.SteriskEmail(isSenderSelf, keyValue, asterisk);
            return Success("操作成功。");
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult EmailUpload()
        {
            ...
            string uploadDate = DateTime.Now.ToString("yyyy年MM月dd日HH:mm:ss");
            ...
            {
                Success = true,
                Data = new { oldName = oldName, newName = nameSTR, path = virtualPath, uploadDate = uploadDate },
                SaveName = nameSTR
            };
            return Content(JsonData.ToJson());
        }
        ...
        public void DownloadFile(string file)
        {
            ...
            string filename = Server.UrlDecode(data["filename"].ToString());//返回客户端文件名称
            string filepath = Server.UrlDecode(this.Server.MapPath("~/" + data["filepath"].ToString()));

...
namespace LeaRun.SystemManage.Service
{
    ...
    /// 描 述：编号规则
    /// </summary>
    public class CodeRuleService : Dao<CodeRuleEntity>, ICodeRuleService
    {
        /// <summary>
        /// 带连接字符串参数的构造函数
        /// </summary>
        /// <param name="dbcontext"></param>
        public CodeRuleService(DbContextBase dbcontext) : base(dbcontext) { }
        ...
        public void SaveForm(string keyValue, CodeRuleEntity codeRuleEntity)
        {
            if (!string.IsNullOrEmpty(keyValue))
            {
                ...
                base.Update(codeRuleEntity);
            }
            else
            {
                codeRuleEntity.Create();
                base.Insert(codeRuleEntity);
            }
        }
        #endregion
        ...
        { 
            ...
            if (coderuleentity != null)
            {
                try
                {
                    int nowSerious = 0;
                    //取得流水号种子
                    List<CodeRuleSeedEntity> codeRuleSeedlist = db.Queryable<CodeRuleSeedEntity>(t => t.RuleId == coderuleentity.RuleId).ToList();
                    //取得最大种子
        ...
        public bool UseRuleSeed(string userId, string moduleId, string enCode )
        {
            ...
            try
            {
                ...
                {
                    //删除用户已经用掉的种子
                    db.Delete<CodeRuleSeedEntity>(t => t.UserId == userId && t.RuleId == coderuleentity.RuleId);
                }
            }
            catch (Exception)
            {

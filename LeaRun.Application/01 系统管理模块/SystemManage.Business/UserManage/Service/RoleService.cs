...
namespace LeaRun.UserManage.Service
{
    ...
    /// 描 述：角色管理
    /// </summary>
    public class RoleService : Dao<RoleEntity>, IRoleService
    {
        ...
        public RoleService(DbContextBase dbcontext) : base(dbcontext)
        {
        ...
        public void SaveForm(string keyValue, RoleEntity roleEntity)
        {
            if (!string.IsNullOrEmpty(keyValue))
            {
                ...
                base.Update(roleEntity);
            }
            else
            {
                roleEntity.Create();
                roleEntity.Category = 1;
                base.Insert(roleEntity);
            }
        }
        #endregion
    }

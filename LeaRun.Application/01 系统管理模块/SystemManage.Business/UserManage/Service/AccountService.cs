...
namespace LeaRun.UserManage.Business
{
    /// <summary>
    /// 版 本
    ...
    /// 描 述：注册账户
    /// </summary>
    public class AccountService : Dao<AccountEntity>, IAccountService
    {
        /// <summary>
        /// 带连接字符串参数的构造函数
        /// </summary>
        /// <param name="dbcontext"></param>
        public AccountService(DbContextBase dbcontext) : base(dbcontext){ }
        ...
        {
            if (base.Queryable(t => t.MobileCode == mobileCode).Count() == 0)
            {
                //验证每个IP 不能获取超过5次
                if (base.Queryable(t => t.IPAddress == Net.Ip).Count() >= 5)
                {
        ...
        {
            var data = base.FindEntity(t => t.MobileCode == accountEntity.MobileCode);
        ...
        public void LoginLimit(string platform, string account, string iPAddress, string iPAddressName)
        {
            ...
            int IsOk = base.ExecuteProc("PROC_verify_IPAddress", parameter);
        }
    }
}
